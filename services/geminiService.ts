...
import { GoogleGenAI, Type } from "@google/genai";
      ...
      export const extractRollNumbersFromImage = async (base64Image: string): Promise<string[]> => {
        ...
        try {
          ...
          throw new Error(error instanceof Error ? error.message : "Failed to extract numbers from the image.");
        }
      };
      ...
      export const extractClassListFromImage = async (base64Image: string): Promise<Student[]> => {
        ...
        try {
          ...
          throw new Error(error instanceof Error ? error.message : "Failed to extract class details from the image.");
        }
      };